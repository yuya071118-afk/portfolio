...
document.querySelectorAll('.mni').forEach(item=>{
  item.addEventListener('click',()=>{
    ...
    const targets={
      fv:0,
      works:cachedWorksTop,
      about:cachedAboutTop,
      contact:cachedContactTop,
    };
    menuOpen=false;
...
function updateMenuColor(){
  const sy=scrollY, fvH=fvWrap.offsetHeight;
  const inLight=sy>fvH*0.4&&(!contactSW||sy<cachedContactTop+contactSW.offsetHeight*0.3);
